...
Go ahead and start up JSON Server by running `json-server --watch db.json` in
your terminal.
...
need to return the `fetch()` chain from your `submitData` function to ensure
...
With this information, you can now use `fetch()` — the browser inside your
browser's JavaScript environment — to send both `GET` and `POST` requests! With
this knowledge we're ready to stitch together server updates (reads **and**
updates) with DOM updating and event handling. We're almost ready to build the
"Simple Liker" from scratch!
